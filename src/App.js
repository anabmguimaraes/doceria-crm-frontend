...
const useData = ({ onNewOrder }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData(currentData => {
        const oldPendingOrdersCount = initialFetchDone.current ? currentData.pedidos.filter(p => p.status === 'Pendente').length : -1;
  ...
  useEffect(() => {
    ...
    resumeRefresh();
    return () => pauseRefresh();
  }, [fetchData, resumeRefresh, pauseRefresh]);
...
function App() {
  ...
  }, [showLogin, confirmDelete.isOpen, lightboxImage, pauseRefresh, resumeRefresh]);
      ...
      <div className="p-4 md:p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <Button onClick={() => window.open('/cardapio.html', '_blank')} className="w-full"><BookOpen className="w-4 h-4" /> Cardápio Delivery</Button>
              <Button onClick={() => window.open('/cardapio-festa.html', '_blank')} className="w-full"><Cake className="w-4 h-4" /> Cardápio de Festas</Button>
          </div>
        </div>
  ...
  const Pedidos = () => {
    ...
    useEffect(() => {
        ...
        return () => resumeRefresh();
    }, [showModal, viewingOrder, pauseRefresh, resumeRefresh]);
