...
const useData = () => {
  ...
  const updateItem = async (section, id, updatedItem) => {
    ...
    setData(prev => ({ ...prev, [section]: prev[section].map(item => (item.id === id ? returnedItem : item)) }));
  };
...
function App() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (authUser) => {
      if (authUser) {
        ...
        setUser({ auth: authUser, role: userDoc.exists() ? userDoc.data().role || 'visitante' : 'visitante' });
        if(currentPage === 'pagina-inicial') setCurrentPage('dashboard');
      } else {
        setUser(null);
  ...
  const handleLogin = async () => {
    ...
    } catch (error) { setLoginError('Email ou senha inválidos.'); }
  };
  ...
  const handleRegister = async () => {
    try {
      ...
      await setDoc(doc(db, "users", userCredential.user.uid), { email: userCredential.user.email, role: "visitante" });
      setShowLogin(false); setEmail(''); setPassword('');
    } catch (error) { setLoginError(error.code === 'auth/email-already-in-use' ? 'Este email já está em uso.' : 'Erro ao registrar.'); }
  };

  const handleLogout = async () => { await signOut(auth); };

  const allMenuItems = [ { id: 'pagina-inicial', label: 'Página Inicial', icon: Home, roles: ['admin', 'visitante', null] }, { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, roles: ['admin', 'visitante'] }, { id: 'clientes', label: 'Clientes', icon: Users, roles: ['admin', 'visitante'] }, { id: 'pedidos', label: 'Pedidos', icon: ShoppingCart, roles: ['admin', 'visitante'] }, { id: 'produtos', label: 'Produtos', icon: Package, roles: ['admin', 'visitante'] }, { id: 'agenda', label: 'Agenda', icon: Calendar, roles: ['admin', 'visitante'] }, { id: 'fornecedores', label: 'Fornecedores', icon: Truck, roles: ['admin', 'visitante'] }, { id: 'relatorios', label: 'Relatórios', icon: BarChart3, roles: ['admin', 'visitante'] }, { id: 'financeiro', label: 'Financeiro', icon: DollarSign, roles: ['admin'] }, { id: 'configuracoes', label: 'Configurações', icon: Settings, roles: ['admin'] }, ];
  ...
  const callGeminiAPI = async (prompt) => {
    ...
    try {
      ...
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!response.ok) throw new Error(`API error: ${response.statusText}`);
      const result = await response.json();
      return result.candidates?.[0]?.content?.parts?.[0]?.text || "Não foi possível gerar o conteúdo.";
    } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <p className="text-gray-600 leading-relaxed mb-4">
                        Somos uma doceria apaixonada por criar momentos doces e inesquecíveis.
                    </p>
                    <div className="space-y-3">
                        ...
                        <p className="flex items-center gap-2 text-gray-700">
                            <MapPin size={20} /> Av. Comercial, 433 - Jd. Nova Esperanca, Goiânia - GO
                        </p>
                    </div>
                </div>
                <div>
  ...
  const Clientes = () => {
    ...
    const handleSubmit = async (e) => { e.preventDefault(); if (editingClient) { await updateItem('clientes', editingClient.id, formData); } else { await addItem('clientes', { ...formData, totalCompras: 0, ultimaCompra: new Date().toISOString().split('T')[0] }); } resetForm(); };
    const handleEdit = (client) => { setEditingClient(client); setFormData(client); setShowModal(true); };
    const handleGenerateBirthdayMessage = async (client) => { const prompt = `Crie uma mensagem de aniversário curta e amigável para ${client.nome}.`; const message = await callGeminiAPI(prompt); setGeneratedContent({ isOpen: true, title: `Mensagem para ${client.nome}`, content: message }); };
    const columns = [ { header: "Cliente", render: (row) => (<div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center text-white font-bold shadow-md">{row.nome.charAt(0).toUpperCase()}</div><div><p className="font-semibold text-gray-800">{row.nome}</p><p className="text-sm text-gray-500">{row.email}</p></div></div>) }, { header: "Telefone", render: (row) => (<span>{row.telefone}</span>) }, { header: "Total Compras", render: (row) => (<span className="font-semibold text-green-600">R$ {(row.totalCompras || 0).toFixed(2)}</span>) }, { header: "Última Compra", render: (row) => new Date(row.ultimaCompra).toLocaleDateString('pt-BR') }, { header: "Status", render: (row) => (<span className={`px-3 py-1 rounded-full text-xs font-medium ${row.status === 'VIP' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'}`}>{row.status}</span>) } ];
    const actions = [ { icon: Sparkles, label: "Gerar Mensagem", onClick: handleGenerateBirthdayMessage }, { icon: Edit, label: "Editar", onClick: handleEdit }, { icon: Trash2, label: "Excluir", onClick: (row) => setConfirmDelete({ isOpen: true, onConfirm: () => deleteItem('clientes', row.id) }) } ];
    return (
      <div className="p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
  ...
  const Produtos = () => {
    ...
    const handleGenerateDescription = async () => { if (!formData.nome || !formData.categoria) return; const prompt = `Crie uma descrição atraente para um(a) "${formData.nome}" da categoria "${formData.categoria}".`; const description = await callGeminiAPI(prompt); setFormData(prev => ({ ...prev, descricao: description })); };
  ...
  const Configuracoes = () => {
    ...
    const [userFormData, setUserFormData] = useState({ email: '', password: '', role: 'visitante' });
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            setUsers(data.filter(u => u.uid !== user.auth.uid));
        } catch (error) {
    ...
    const resetUserForm = () => {
        ...
        setEditingUser(null);
        setUserFormData({ email: '', password: '', role: 'visitante' });
    };
    ...
    const handleUserSubmit = async (e) => {
        ...
        try {
            if (editingUser) {
                await fetch(`${API_BASE_URL}/users/${editingUser.uid}/role`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ role: userFormData.role }),
                });
                if (userFormData.password) {
                    await fetch(`${API_BASE_URL}/users/${editingUser.uid}/password`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
    ...
    const handleEditUser = (userToEdit) => {
        ...
        setUserFormData({ email: userToEdit.email, password: '', role: userToEdit.role });
    ...
    const actions = [
        ...
        { icon: Trash2, label: "Excluir", onClick: (row) => setConfirmDelete({ isOpen: true, onConfirm: () => handleDeleteUser(row.uid) }) }
    ];
